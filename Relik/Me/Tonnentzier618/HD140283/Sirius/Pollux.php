<?php
// ─────────────────────────────────────────────────────────────────────────────
//  Pollux.php — Market Dashboard
//  Saldo dari : Xano API → /formalhault  (Nama, Saldo, id, RegNo)
//  Item dari  : /data/session-market.json  (sama seperti altair.php)
//  Log beli   : INSERT INTO spica (Dari, Terima, Monetasi, Tujuan, ...)
//  Path saran : /HD140283/Sirius/Pollux.php
// ─────────────────────────────────────────────────────────────────────────────
...
/* ── Xano API ───────────────────────────────────────────────────────────────── */
define('XANO_BASE', 'https://x8ki-letl-twmt.n7.xano.io/api:X6h8irt0');

/**
 * GET  /formalhault        → returns array of {id, Nama, Saldo, RegNo, ...}
 * PATCH /formalhault/{id}  → update Saldo for one record
 */
function xano_get_users(): array {
    $ch = curl_init(XANO_BASE . '/formalhault');
...
function xano_patch_saldo(int $id, int $saldo_baru): bool {
    $ch = curl_init(XANO_BASE . '/formalhault/' . $id);
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_CUSTOMREQUEST  => 'PATCH',
        CURLOPT_POSTFIELDS     => json_encode(['Saldo' => $saldo_baru]),
        CURLOPT_HTTPHEADER     => ['Content-Type: application/json'],
        CURLOPT_TIMEOUT        => 10,
    ]);
    $raw  = curl_exec($ch);
...
/* ── Ambil daftar nama & saldo dari Xano ───────────────────────────────────── */
$xano_users_raw = xano_get_users();   // [{id,Nama,Saldo,RegNo,...}, ...]
usort($xano_users_raw, fn($a,$b) => strcmp($a['Nama'], $b['Nama']));

// $users   : Nama  => Saldo   (for template rendering, backward-compat)
// $user_ids: Nama  => Xano id (needed for PATCH on purchase)
$users    = [];
$user_ids = [];
foreach ($xano_users_raw as $r) {
    $users[$r['Nama']]    = (int)$r['Saldo'];
    $user_ids[$r['Nama']] = (int)$r['id'];
}
...
if (isset($_POST['ajax_buy'])) {
    ...
    if (!isset($users[$pembeli])) {
        // Reload dari Xano
        $fresh = xano_get_users();
        foreach ($fresh as $r) {
            if ($r['Nama'] === $pembeli) {
                $users[$pembeli]    = (int)$r['Saldo'];
                $user_ids[$pembeli] = (int)$r['id'];
                break;
            }
        }
        if (!isset($users[$pembeli])) { echo json_encode(['ok'=>false,'msg'=>'Nama pembeli tidak valid.']); exit; }
    }
    ...
    // 1. Debit saldo pembeli via Xano
    $saldo_baru  = $saldo - $total;
    $xano_id     = $user_ids[$pembeli] ?? 0;
    if (!$xano_id || !xano_patch_saldo($xano_id, $saldo_baru)) {
        echo json_encode(['ok'=>false,'msg'=>'Gagal memperbarui saldo di Xano.']); exit;
    }
...
if (isset($_GET['ajax_saldo'])) {
    ...
    $fresh = xano_get_users();
    foreach ($fresh as $r) {
        if ($r['Nama'] === $nama) { $saldo = (int)$r['Saldo']; break; }
    }
...
// Saldo dari Xano API (live saat halaman dimuat)
...
async function submitBuy() {
    ...
    try {
        ...
        const res  = await fetch('Pollux.php', { method:'POST', body: fd });
